...
export async function getCandles(
  ...
  if(isForex(symbol)){ const p = await polygonFetch(symbol, tf, n, perCall); if(p.length) return p; }

  // 3) Finnhub (forex only)
  if(isForex(symbol)){ const f = await finnhubFetch(symbol, tf, n, perCall); if(f.length) return f; }

  // 4) Synthetic from nearest TF (best-effort)
  return await syntheticFromNearest(symbol, tf, n, perCall);
}
...
// TwelveData aliases to increase hit-rate
...
function clone(c:Candle):Candle{ return {t:c.t,o:c.o,h:c.h,l:c.l,c:c.c}; }
