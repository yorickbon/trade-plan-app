...
// Create a timeout signal that works across Node versions
function withTimeout(ms: number): AbortSignal | undefined {
  ...
  if (anyAbort && typeof anyAbort.timeout === "function") {
    try { return anyAbort.timeout(ms); } catch {}
  }
...
function originFromReq(req: NextApiRequest) {
  ...
  const proto = host.includes("localhost") || host.startsWith("127.") ? "http" : (protoHeader || "https");
...
// SECTION 8 — AI meta + sanity helpers
...
async function tdSeries15(pair: string): Promise<Series | null> {
  ...
  try {
    ...
    const r = await fetch(url, { cache: "no-store", signal: withTimeout(2500) });
    if (!r.ok) return null;
...
// SECTION 10 — COT cue from headlines (optional)
type CotCue = {
...
function detectCotCueFromHeadlines(headlines: AnyHeadline[]): CotCue | null {
  ...
  // Flatten titles + descriptions for simple regex scanning
  const text = headlines
    .map((h) => `${h?.title || ""} ${h?.description || ""}`)
    .join(" • ")
    .toLowerCase();

  // Only attempt if COT/CFTC explicitly mentioned
  const mentionsCot = /(commitments?\s+of\s+traders|cot|cftc)\b/.test(text);
  if (!mentionsCot) return null;

  // Currency term patterns (broad to catch aliases)
  ...
  for (const [cur, regs] of Object.entries(terms)) {
    if (regs.some((re) => re.test(text))) {
      // Look for explicit net long/short mentions near the currency term
      ...
      // Or changes in positioning (increase long/short)
  ...
  const summaryParts = Object.entries(net).map(
  ...
  return {
    method: "headline_fallback",
    reportDate: null,
    summary: `COT cues (headlines): ${summaryParts.join(", ")}`,
    net,
  };
}
// SECTION 11 — Sentiment snapshot
...
// SECTION 12 — Calendar helpers (OCR analysis + relevance)

function goodIfHigher(title: string): boolean | null {
  const t = String(title || "").toLowerCase();

  // Growth / inflation / activity — "higher is good"
  ...
  // Labor — lower unemployment/claims tends to be good
  if (/unemployment|jobless|initial claims|continuing claims/.test(t)) return false;

  // Employment growth — higher jobs/payrolls is good
  if (/(nonfarm|nfp|employment change|payrolls|jobs)/.test(t)) return true;

  // External balances / policy — treat higher as better here
...
function evidenceLine(it: any, cur: string): string | null {
  ...
  const comps: string[] = [];
  if (f != null) comps.push(a < f ? "< forecast" : a > f ? "> forecast" : "= forecast");
  if (p != null) comps.push(a < p ? "< previous" : a > p ? "> previous" : "= previous");

  let verdict = "neutral";
  if (dir === true) {
    verdict =
      a > (f ?? a) && a > (p ?? a) ? "bullish" :
      a < (f ?? a) && a < (p ?? a) ? "bearish" : "mixed";
  } else if (dir === false) {
    verdict =
      a < (f ?? a) && a < (p ?? a) ? "bullish" :
      a > (f ?? a) && a > (p ?? a) ? "bearish" : "mixed";
  ...
  const suffix = comps.length ? ` ${comps.join(" and ")}` : "";
  return `${cur} — ${String(it?.title || "").trim()}: actual ${a}${suffix} → ${verdict} ${cur}`;
}
...
function analyzeCalendarOCR(
  ...
  function addScore(cur: string, verdict: string) {
    if (!cur) return;
    if (!(cur in perScore)) perScore[cur] = 0;
    if (verdict.includes("bullish")) perScore[cur] += 1;
    else if (verdict.includes("bearish")) perScore[cur] -= 1;
  }

  let warnMinutes: number | null = null;

  for (const it of ocr?.items ?? []) {
    // Next 60m high-impact warning
    if (String(it?.impact || "") === "High" && it?.timeISO) {
      const t = Date.parse(it.timeISO);
      if (Number.isFinite(t) && t >= nowMs) {
        const mins = Math.floor((t - nowMs) / 60000);
        if (mins <= 60) warnMinutes = warnMinutes == null ? mins : Math.min(warnMinutes, mins);
      }
    }
    ...
    if (it?.timeISO) {
      const t = Date.parse(it.timeISO);
      if (Number.isFinite(t) && t <= nowMs && t >= nowMs - 72 * 3600 * 1000) {
  ...
  const bLab = label(perScore[base] ?? 0);
  const qLab = label(perScore[quote] ?? 0);
  ...
  const biasLine =
    `Calendar bias for ${pair}: ` +
    `${instr ? `Instrument: ${instr}; ` : ""}` +
    `Per-currency: ${base}:${bLab} / ${quote}:${qLab}`;

  const biasNote =
    `Per-currency: ${base} ${bLab} vs ${quote} ${qLab}` +
    (instr ? `; Instrument bias: ${instr}` : "");

  return { biasLine, biasNote, warningMinutes: warnMinutes, evidenceLines: lines };
}

// Currency helpers for OCR usability checks
...
function relevantCurrenciesFromInstrument(instr: string): string[] {
  ...
  // Default to USD when instrument is ambiguous or single-currency
  if (U.endsWith("USD") || U.startsWith("USD")) return ["USD"];
  return ["USD"];
}

function hasUsableFields(r: any): boolean {
  // Present if actual exists AND (forecast OR previous) exists
  return r != null && r.actual != null && (r.forecast != null || r.previous != null);
}
// SECTION 13 — API calendar fallback
async function fetchCalendarRaw(
  ...
  try {
    ...
    const r = await fetch(url, { cache: "no-store", signal: withTimeout(5000) });
...
async function fetchCalendarForAdvisory(
  ...
  try {
    ...
    const r = await fetch(url, { cache: "no-store", signal: withTimeout(4000) });
    const j: any = await r.json().catch(() => ({}));
...
// SECTION 14 — Composite bias (calendar + headlines + CSM)
...
// SECTION 15 — Prompt system (NO-GUESS + forced order + consistency)
function systemCore(
  instrument: string,
...
// SECTION 17 — Enforcement helpers
  ...
  const messages = [
    {
      role: "system",
      content:
        "Add a compliant **Option 2 (Alternative)**. Keep everything else unchanged. Include Direction, Order Type, explicit Trigger, Entry (zone or single), SL, TP1/TP2, Conviction %.",
    },
    {
      role: "user",
  ...
  const messages = [
    {
      role: "system",
      content:
        "Insert a labeled 'Option 1 (Primary)' block BEFORE 'Option 2'. Use the primary trade details already present. Include Direction, Order Type, Trigger, Entry (zone or single), SL, TP1/TP2, Conviction %. Keep other content unchanged.",
    },
    {
      role: "user",
  ...
  const messages = [
    {
      role: "system",
      content:
        "Add a 'Quick Plan (Actionable)' section at the very top, copying primary trade details. Keep all other sections unchanged and in order.",
    },
    {
      role: "user",
...
// SECTION 18 — Consistency guard (post-processing)
function applyConsistencyGuards(
  ...
  let out = text || "";
  const signs = [args.headlinesSign, args.csmSign, args.calendarSign].filter(
    (s) => s !== 0
  );
  ...
  if (reTF.test(out)) {
    out = out.replace(
      reTF,
      (_, p1) => `${p1}${aligned ? "Match" : mismatch ? "Mismatch" : "Match"}`
    );
  } else {
    ...
    if (anchor.test(out)) {
      out = out.replace(
        ...
        (m) =>
          `${m}\n(Consistency: ${
            aligned ? "Match" : mismatch ? "Mismatch" : "Match"
          })`
      );
    }
  }
  ...
  try {
    ...
    if (mode === "expand") {
      ...
      const footer = buildServerProvenanceFooter({
        ...
        calendar_status: (c.calendar ? "image-ocr" : (calAdv?.status ?? "unavailable")) as
          | "api"
          | "image-ocr"
          | "unavailable",
      ...
      return res
        .status(200)
        .json({ ok: true, text, meta: { instrument: c.instrument, cacheKey, model: modelExpand, vp_version: VP_VERSION } });
    }
    ...
    if (!isMultipart(req)) {
      return res.status(400).json({
        ok: false,
        reason:
          "Use multipart/form-data with files: m15, h1, h4 (PNG/JPG/WEBP) and optional 'calendar'. Or pass m15Url/h1Url/h4Url (TradingView/Gyazo links) and optional 'calendarUrl'. Include 'instrument'.",
      });
    }
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(`[vision-plan] parsed in ${Date.now() - tParse}ms`);
    }
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `[vision-plan] images ready ${Date.now() - tImg}ms (m15=${dataUrlSizeBytes(m15)}B, h1=${dataUrlSizeBytes(h1)}B, h4=${dataUrlSizeBytes(h4)}B, cal=${dataUrlSizeBytes(calUrlOrig)}B)`
      );
    }
    if (!m15 || !h1 || !h4) {
      return res.status(400).json({
