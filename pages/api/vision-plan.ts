...
//
// Accuracy-first: HQ image pipeline ON by default (VISION_HQ=1 or unset):
//  - 1280px width, mild sharpen()
//  - JPEG q≈82; if large, try WebP q≈90/82 (OpenAI supports webp)
//  - Best-effort cap ~≤900 KB to keep Y-axis digits crisp
// Fast path (VISION_HQ=0): 1100px, JPEG q≈68, ≤450 KB
//
// Headlines: fetch 12, pass only 6 into the model prompt.
// Enforcement: **restored to original base** (3 small passes), plus a safe output sanitizer
// to trim any model preface and start at "Quick Plan (Actionable)".
//
...
const HQ_WEBP_Q1 = 90;
const HQ_WEBP_Q2 = 82;
const HQ_TARGET_MAX = 900 * 1024; // ~≤900 KB

const FAST_MAX_WIDTH = 1100;
const FAST_JPEG_Q = 68;
const FAST_MIN_Q = 50;
const FAST_TARGET_MAX = 450 * 1024;
...
async function processAccurate(buf: Buffer): Promise<{ buffer: Buffer; mime: "image/jpeg" | "image/webp" }> {
  // Try JPEG q≈82 first
  let out = await toJpeg(buf, HQ_MAX_WIDTH, HQ_JPEG_Q);
  let mime: "image/jpeg" | "image/webp" = "image/jpeg";
  ...
  if (out.byteLength > HQ_TARGET_MAX) {
    ...
    if (webp1.byteLength <= HQ_TARGET_MAX) {
      ...
      const best = [
        { b: out, m: "image/jpeg" as const },
        { b: webp1, m: "image/webp" as const },
        { b: webp2, m: "image/webp" as const },
...
async function processFast(buf: Buffer): Promise<{ buffer: Buffer; mime: "image/jpeg" }> {
  // pass 1
  let q = FAST_JPEG_Q;
...
async function fetchImageDataUrlFromLink(link: string): Promise<string | null> {
  ...
  try {
    ...
    if (ctype.startsWith("image/")) {
      const dataUrl = await processBufferToDataUrl(first.buf);
      if (process.env.NODE_ENV !== "production") {
        console.log(`[vision-plan] link processed size=${dataUrlSizeBytes(dataUrl)}B (hq=${VISION_HQ})`);
      }
      return dataUrl;
    }
    ...
    const dataUrl = await processBufferToDataUrl(second.buf);
    if (process.env.NODE_ENV !== "production") {
      console.log(`[vision-plan] og:image processed size=${dataUrlSizeBytes(dataUrl)}B (hq=${VISION_HQ})`);
    }
    return dataUrl;
  } catch {
    return null;
...
function withFetchTimeout(url: string, ms: number, init?: RequestInit) {
...
// ---------- sanitizer to ensure clean card start ----------
function sanitizeCardOutput(text: string): string {
  if (!text) return text;
  const anchor = "Quick Plan (Actionable)";
  const i = text.indexOf(anchor);
  if (i > 0) {
    // Trim any preface (like "Instrument: ...\nai_meta ...\nCard:")
    return text.slice(i);
  }
  return text;
}

// ---------- headlines fetcher (12→6 into prompt) ----------
...
async function fetchedHeadlines(req: NextApiRequest, instrument: string) {
  try {
    ...
    const lines = items
      .slice(0, 6) // <= only 6 go into the model
      .map((it: any) => {
  ...
  try {
    ...
    if (!isMultipart(req)) {
      return res.status(400).json({
        ok: false,
        reason:
          "Use multipart/form-data with files: m15, h1, h4 (PNG/JPG/WEBP) and optional 'calendar'. Or pass m15Url/h1Url/h4Url (TradingView links). Also include 'instrument' field.",
      });
    }
    ...
    // URLs (optional; TradingView / direct image link)
    ...
    // Build images: prefer file if present; otherwise use URL fetcher (always processed via HQ/fast pipeline)
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `[vision-plan] sizes: m15=${dataUrlSizeBytes(m15)}B, h1=${dataUrlSizeBytes(h1)}B, h4=${dataUrlSizeBytes(h4)}B, cal=${dataUrlSizeBytes(calUrl)}B (hq=${VISION_HQ})`
      );
    }
    ...
    if (!m15 || !h1 || !h4) {
      return res
        .status(400)
        .json({ ok: false, reason: "Provide all three charts: m15, h1, h4 — either as files or valid TradingView/direct image links." });
    }
    ...
    // 5) Sanitize output to ensure card starts at "Quick Plan (Actionable)"
    text = sanitizeCardOutput(text);
