 ...
 *
 * Baseline: Previous approved version with Option 2 enforcement, Calendar warning + bias/evidence,
 * Headlines+CSM sentiment, headlines-only COT cue, and single-pass fixer.
 *
 * ✅ New in this patch (compat-focused):
 * - Replaced all uses of `Promise.any` with a local shim: `promiseAnyCompatNullable`.
 * - Removed all uses of `AbortSignal.timeout(...)` (not in Node < 18):
 *   All network calls now go through `fetchWithTimeout(url, ms, extraHeaders?)`.
 *
 * ✅ Still includes:
 * - Headlines bias computation (+ provenance: headlines_bias_label/score).
 * - COT headline cue detector; if none found ⇒ prints “COT news not found.” and provenance shows cot_used=false.
 * - Consolidated single-pass fixer (`consolidateFixes`) + `passes` switch (multipart field).
 * - Parallel provider attempts for CSM time series using the shim.
 */
...
// Speed knobs
const DEFAULT_PASSES = Number(process.env.VISION_ENFORCE_PASSES ?? 1); // 0=skip fixer, >=1=run one pass
...
// UNIVERSAL timeout-friendly fetch (no AbortSignal.timeout)
async function fetchWithTimeout(url: string, ms: number, headers?: Record<string, string>) {
  const ac = new AbortController();
  const id = setTimeout(() => ac.abort(), Math.max(500, ms | 0));
  try {
...
// ---------- fetchSeries15 (parallel providers via shim) ----------
async function fetchSeries15(pair: string): Promise<Series | null> {
  ...
  const wrapped = attempts.map((p) =>
    p.then((r) => (r && Array.isArray(r.c) && r.c.length ? r : Promise.reject("empty")))
  );
...
async function fetchCalendarForAdvisory(
  req: NextApiRequest,
  instrument: string
): Promise<{ text: string | null, status: "api" | "unavailable", provider: string | null, warningMinutes: number | null, advisoryText: string | null, biasNote: string | null, raw?: any | null, evidence?: string[] | null }> {
  try {
    const base = originFromReq(req);
...
function messagesFull(args: {
  ...
  const system = [
    ...
    "OUTPUT format:", "Quick Plan (Actionable)", "",
  ...
  return [
    ...
    { role: "user", content: buildUserPartsBase({
        ...
        calendarDataUrl: args.calendarDataUrl, calendarText: args.calendarText, headlinesText: args.headlinesText,
        sentimentText: args.sentimentText, calendarAdvisoryText: args.calendarAdvisory?.advisoryText || null,
...
function messagesFastStage1(args: {
  ...
  const system = [
    ...
    `  "breakoutProof": { "bodyCloseBeyond": boolean, "retestHolds": boolean, "sfpReclaim": boolean },`,
    `  "candidateScores": [{ "name": string, "score": number, "reason": string }],`,
    `  "sources": { "headlines_used": number, "headlines_instrument": string, "headlines_provider": string | null, "calendar_used": boolean, "calendar_status": string, "calendar_provider": string | null, "csm_used": boolean, "csm_time": string, "cot_used": boolean, "cot_report_date": string | null, "cot_error": "no cot cues" | string | null, "cot_method": string | null, "calendar_warning_minutes": number | null, "calendar_bias_note": string | null, "calendar_evidence": string[] | null, "headlines_bias_label": "bullish" | "bearish" | "neutral" | "unavailable", "headlines_bias_score": number | null, "cot_bias_summary": string | null } }`,
    "\n",
  ].join("\n");

  const parts = buildUserPartsBase({
    instrument: args.instrument, dateStr: args.dateStr, m15: args.m15, h1: args.h1, h4: args.h4,
    calendarDataUrl: args.calendarDataUrl, calendarText: !args.calendarDataUrl && args.calendarText ? args.calendarText : undefined,
...
function messagesExpandStage2(args: {
  ...
  const system = [
    ...
    "Sections to output:",
    "Full Breakdown", "• Technical View (HTF + Intraday): 4H/1H/15m structure",
    "• Fundamental View (Calendar + Sentiment + Headlines):",
    "• Tech vs Fundy Alignment: Match | Mismatch (+why)",
...
// ---------- legacy targeted fixers (kept for reference) ----------
...
async function enforceOption2(instrument: string, text: string) {
  ...
  const messages = [
    { role: "system", content:
      `Add a compliant **Option 2** to this trade card. Keep Option 1 exactly as-is (numbers and wording). Option 2 must be a secondary structured play (e.g., Break + Retest, Trendline break, SFP), and MUST include:
  ...
  try {
    ...
    if (!isMultipart(req)) {
      return res.status(400).json({
        ok: false,
        reason: "Use multipart/form-data with files: m15, h1, h4 (PNG/JPG/WEBP) and optional 'calendar'. Or pass m15Url/h1Url/h4Url (TradingView/Gyazo links). Also include 'instrument' field.",
      });
    }
    ...
    if (calUrl) {
      calendarStatus = "image"; calendarProvider = "image"; calendarText = calAdv.text;
    } else {
      calendarText = calAdv.text; calendarStatus = calAdv.status; calendarProvider = calAdv.provider;
    }

    // Sentiment (CSM + headlines bias + COT cue)
    let csm: CsmSnapshot;
    ...
    // Single-pass fixes for full path
    {
      const bp = aiMeta?.breakoutProof || {};
      const breakoutClaim = String(aiMeta?.selectedStrategy || "").toLowerCase().includes("breakout");
      const missingProof = !(bp?.bodyCloseBeyond === true && (bp?.retestHolds === true || bp?.sfpReclaim === true));
