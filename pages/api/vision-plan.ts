...
function systemCore(
 ...
 const baseLines = [
  ...
  "Multi-timeframe roles (fixed):",
    ...
    "",
   "MANDATORY CHART ANALYSIS PROTOCOL - SHOW ALL WORK:",
    "",
    "FOR EACH TIMEFRAME YOU MUST:",
    "",
    "Step 1: PRICE ANCHORS (Report exact prices)",
    "- Leftmost visible candle close: X.XXXXX",
    "- Rightmost visible candle close: Y.YYYYY", 
    "- Highest point on entire chart: Z.ZZZZZ",
    "- Lowest point on entire chart: W.WWWWW",
    "- Current price from API: [use provided]",
    "",
    "Step 2: SWING STRUCTURE (List every swing with prices)",
    "- Identify ALL swing highs (local peaks): e.g., 'SH1: 0.6690, SH2: 0.6710, SH3: 0.6650'",
    "- Identify ALL swing lows (local troughs): e.g., 'SL1: 0.6400, SL2: 0.6380, SL3: 0.6420'",
    "- Count rising swings: If SH2 > SH1 AND SL2 > SL1 (continue pattern)",
    "- Count falling swings: If SH2 < SH1 AND SL2 < SL1 (continue pattern)",
    "- Verdict: If rising count > falling count = UPTREND, else if falling > rising = DOWNTREND, else RANGE",
    "",
    "Step 3: BOS DETECTION (Last 10 candles only - use TradingView data if available)",
    "- Most recent swing high BEFORE last 10 candles: X.XXXXX",
    "- Did any of last 10 candles CLOSE above it? YES/NO → If YES: BOS UP at [price]",
    "- Most recent swing low BEFORE last 10 candles: Y.YYYYY", 
    "- Did any of last 10 candles CLOSE below it? YES/NO → If YES: BOS DOWN at [price]",
    "- Report: 'BOS UP at X.XXXX' or 'BOS DOWN at X.XXXX' or 'No recent BOS'",
    "",
    "Step 4: PRICE LOCATION (Mathematical)",
    "- Range = Highest - Lowest",
    "- Current position = (Current - Lowest) / Range × 100",
    "- If > 75%: 'At recent highs' | If < 25%: 'At recent lows' | Else: 'Middle range'",
    "",
    "Step 5: CONFLUENCE CHECK",
    "- Does TradingView BOS data match your analysis? Report any discrepancies.",
    "- Is current API price within 0.3% of chart's rightmost candle? If NO, charts may be outdated.",
    "",
    "EXAMPLE OUTPUT FORMAT:",
    "4H: Left=0.6420, Right=0.6605, High=0.6710, Low=0.6380",
    "    Swings: SH[0.6690,0.6710,0.6650] SL[0.6400,0.6380,0.6420]",
    "    Pattern: 2 higher highs, 1 higher low vs 1 lower = UPTREND",
    "    BOS: Last 10 candles broke above 0.6650 SH = BOS UP at 0.6655",
    "    Location: (0.6605-0.6380)/(0.6710-0.6380) = 68% = Middle-high range",
    "    TradingView confirms: BOS UP ✓ | API price 0.66053 = 0.8% from chart ✓",
    "",
    "CRITICAL: If you cannot identify specific swing prices, state 'CHART UNREADABLE - need y-axis visible' and STOP.",
