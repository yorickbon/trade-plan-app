/**
 * CHANGE MANIFEST — /pages/api/vision-plan.ts (latency-optimized, compile-ready)
 *
 * Perf/compat changes:
 * - All external calls use explicit timeouts (no AbortSignal.timeout usage).
 * - Headlines, calendar, CSM, and live price fetch in PARALLEL with image work.
 * - Promise.any replaced by local shim (promiseAnyCompatNullable) for TS/target safety.
 * - Images tuned smaller by default (env overridable).
 * - Consolidated fixer is ON by default (DEFAULT_PASSES=1) → guaranteed Option 2.
 * - Optional `slim=1` field to skip 4H (you can ignore; default still requires 4H).
 *
 * Functional:
 * - Headlines bias summary (bullish/bearish/neutral + avg) added to sentiment.
 * - COT cue via headlines; prints "COT news not found." when absent.
 * - Provenance includes headlines bias + COT cue info.
 *
 * API/output shape unchanged (card sections, names, and tables retained).
 */
...
const HEADLINES_LIMIT = Number(process.env.VISION_HEADLINES_LIMIT ?? 6);
...
// Default consolidated fixer passes (1 = enforce Option 2 by default)
...
async function fetchWithTimeout(url: string, ms: number, init?: RequestInit) {
  ...
  try {
    const r = await fetch(url, { ...(init || {}), signal: ac.signal, redirect: "follow", cache: "no-store" } as any);
    return r;
...
async function fetchCalendarForAdvisory(req: NextApiRequest, instrument: string) {
  try {
    ...
    if (j?.ok) {
      ...
      const advisory = [ warn != null ? `⚠️ High-impact event in ~${warn} min.` : null, bias ? `Recent result alignment: ${bias}.` : null ]
        .filter(Boolean).join("\n");
...
function messagesFull(args: {
  ...
  "sources": { "headlines_used": number, "headlines_instrument": string, "headlines_provider": string | null,
               ...
               "headlines_bias_label": "bullish" | "bearish" | "neutral" | "unavailable", "headlines_bias_score": number | null,
...
async function callOpenAI(messages: any[]) {
  const rsp = await fetchWithTimeout(`${OPENAI_API_BASE}/chat/completions`, OPENAI_TIMEOUT_MS, {
    method: "POST",
    headers: { "content-type": "application/json", authorization: `Bearer ${OPENAI_API_KEY}` },
    body: JSON.stringify({ model: OPENAI_MODEL, messages }),
  });
  const json = await rsp?.json().catch(() => ({} as any));
  if (!rsp || !rsp.ok) throw new Error(`OpenAI vision request failed: ${rsp?.status} ${JSON.stringify(json)}`);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Ok | Err>) {
  try {
    ...
    // Start parallel non-image work immediately
    ...
    // Await parallel non-image promises
    ...
    // Provenance we pass to model (also embedded in ai_meta)
    const provForModel = {
    ...
    if (mode === "fast") {
      ...
      // Consolidated fixer (on by default) but only if needed
      const bp = aiMeta?.breakoutProof || {};
    ...
    // Optional single fixer pass on full path too (DEFAULT_PASSES governs)
    {
      const bp = aiMeta?.breakoutProof || {};
      const breakoutClaim = String(aiMeta?.selectedStrategy || "").toLowerCase().includes("breakout");
      const missingProof = !(bp?.bodyCloseBeyond === true && (bp?.retestHolds === true || bp?.sfpReclaim === true));
