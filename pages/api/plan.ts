...
// IMPORTANT: sentiment is a NAMED export (no default)
...
// ---------- sentiment helper (handles sync/async, number/object) ----------
  ...
  try {
    ...
    const blackout = (() => {
      ...
      return calItems.some(ev => {
        ...
        const hi = (ev.impact ?? "").toLowerCase().includes("high");
        return Number.isFinite(ts) && Math.abs(ts - now) <= within && hi;
      });
    })();
    ...
    if (all.missing.length) {
      const triedAltNote = all.triedAlt ? ` (tried alt symbol: ${all.triedAlt})` : "";
      const sec = Math.round(Number(process.env.PLAN_CANDLES_TIMEOUT_MS ?? 8000) / 1000);
      const out: ApiOut = {
        ok: true,
        plan: {
          text: `Standing down: Missing candles for ${all.missing.join(", ")}${triedAltNote}. Timed out after ${sec}s.`,
          conviction: 60,
    ...
    const fibMid  = swingHi - 0.618 * range; // “0.618 pullback” measured top→bottom
    ...
    if (upBias === "Buy") {
      entry = Number(fibMid.toFixed(5));
      stop  = Number((swingLo - 0.25 * range).toFixed(5));
      tp1   = Number((entry + 0.25 * range).toFixed(5));
      tp2   = Number((entry + 0.50 * range).toFixed(5));
      // Guard rail: enforce stop < entry < tp1 < tp2
      if (!(stop < entry && entry < tp1 && tp1 < tp2)) {
        const e = last.c;
        entry = Number(e.toFixed(5));
        stop  = Number((e - 0.25 * range).toFixed(5));
        tp1   = Number((e + 0.25 * range).toFixed(5));
        tp2   = Number((e + 0.50 * range).toFixed(5));
      }
    } else {
      // Sell
      entry = Number((swingLo + 0.618 * range).toFixed(5)); // mirror
    ...
    try {
      ...
      const val = await scoreHeadlines(recentText);
    ...
    const op1h = (t1h < 0 && wantUp) || (t1h > 0 && !wantUp);
    const op4h = (t4h < 0 && wantUp) || (t4h > 0 && !wantUp);

    let timeFramePenalty = 0;
    if (op1h && op4h) timeFramePenalty += 10;
    else if (op1h || op4h) timeFramePenalty += 5;
