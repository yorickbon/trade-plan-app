...
type Headline = {
  ...
  url: string;
  source: string;
  seen?: string; // ISO timestamp (we surface this from /api/news)
};
...
export default async function handler(
  ...
  // date window for headlines (default 48h)
  ...
  // ---------- fetch candles (15m required; 1H/4H optional) ----------
  const [m4, h1, m15] = await Promise.all<Candle[]>([
    getCandles(instr.code, "4h", 200),
    getCandles(instr.code, "1h", 200),
    getCandles(instr.code, "15m", 200),
  ]);

  const have15 = Array.isArray(m15) && m15.length > 0;
  const haveH1 = Array.isArray(h1) && h1.length > 0;
  const have4H = Array.isArray(m4) && m4.length > 0;

  if (!have15) {
    const out: ApiOut = {
      ok: false,
      reason: "Missing 15m candles",
  ...
  // ---------- lightweight headline sentiment → bias note (does not block) ----------
  let newsBias = "Neutral";
  let newsScore = 0;

  try {
    const recentText = recent.map(h => h.title).join(" • ");
    const score = await scoreSentiment(recentText); // returns number in [-1,1]
