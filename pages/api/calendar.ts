 ...
 *
 * Provider order (by default):
 *   1) FairEconomy / ForexFactory RSS  (free, no key)
 *   2) TradingEconomics guest (fallback/merge)  (TE_API_USER/TE_API_PASS; defaults to guest/guest)
 *
 * Features
 *   - Returns events around a day (±windowHours) with basic fields and a 90m blackout flag for High impact.
 *   - Computes a simple fundamental bias per currency from Actual vs Forecast/Previous with impact weighting.
 *   - Optionally computes an instrument bias (base−quote) when instrument or currencies are provided.
 *   - Clear provenance: provider: "fair" | "te" | "mixed"; if both fail, { ok:false, reason }.
 *
 * Query params
 *   - date=YYYY-MM-DD            (default: today)
 *   - windowHours=48             (6..168)
 *   - instrument=EURUSD          (auto-derives currencies)
 *   - currencies=EUR,USD         (used if instrument missing/invalid)
 *
 * Response (Ok)
 *   {
 *     ok: true,
 *     provider: "fair" | "te" | "mixed",
 *     date: "YYYY-MM-DD",
 *     count: number,
 *     items: CalItem[],                // sorted newest→oldest
 *     bias: {
 *       windowHours: number,
 *       perCurrency: Record<string, BiasSummary>,
 *       instrument?: { pair: string, score: number, label: BiasLabel }
 *     }
 *   }
 *
 * Response (Err)
...
function blackout(iso: string, minutes = 90) {
  const t = new Date(iso).getTime();
  return {
    from: new Date(t - minutes * 60000).toISOString(),
    to: new Date(t + minutes * 60000).toISOString(),
  };
}
...
function goodIfHigher(title: string): boolean | null {
  ...
  // Inflation/prices: higher → hawkish → bullish FX
  if (/(cpi|core cpi|ppi|inflation)/.test(t)) return true;

  // Growth/demand: higher bullish
  if (
    /(gdp|retail sales|industrial production|manufacturing production|consumer credit|housing starts|building permits|durable goods)/
      .test(t)
  )
    return true;

  // PMI/ISM/Confidence: higher bullish
  ...
  if (/unemployment|jobless|jobless claims|initial claims|continuing claims/.test(t))
    return false; // lower is bullish
  if (/(nonfarm|nfp|employment change|payrolls|jobs)/.test(t)) return true;

  // Trade balance: treat higher (less negative) as bullish
  if (/trade balance|current account/.test(t)) return true;

  // Policy rates: higher rate is bullish; cuts bearish
  if (/interest rate|rate decision|refi rate|deposit facility|bank rate|cash rate|ocr/.test(t))
    return true;

  return null; // unknown → neutral handling
}
...
async function fetchFairEconomy(
  ...
  try {
    ...
    while ((m = re.exec(xml))) {
      ...
      // FF doesn't give us explicit event times per item; pubDate is a safe proxy here.
      ...
      const impact: Impact = /High/i.test(title + " " + cat)
        ? "High"
        : /Medium/i.test(title + " " + cat)
        ? "Medium"
        : /Low/i.test(title + " " + cat)
        ? "Low"
        : "None";

      const currency =
        (title.match(/(USD|EUR|GBP|JPY|AUD|NZD|CAD|CHF|CNY|XAU)/i)?.[1] || "")
...
async function fetchTradingEconomics(
  ...
  try {
    ...
    for (const e of arr) {
      ...
      const imp: Impact =
        /high/i.test(String(e?.Importance || ""))
          ? "High"
          : /medium/i.test(String(e?.Importance || ""))
          ? "Medium"
          : /low/i.test(String(e?.Importance || ""))
          ? "Low"
          : "None";
...
function computeBias(
  ...
  function add(
    ...
    if (!per[cur])
      per[cur] = { score: 0, count: 0, label: "neutral", evidence: [] };
...
export default async function handler(
  ...
  try {
    ...
    const windowHours = Math.max(
      6,
      Math.min(168, Number(req.query.windowHours || 48))
    ); // 6h..7d
    ...
    if (instrument && instrument.length >= 6) {
      ...
      currencies = String(req.query.currencies)
        .split(",")
    ...
    if (!items.length) {
      return res
        .status(200)
        .json({ ok: false, reason: "No calendar items from providers" });
    }

    // apply blackout (±90m) for High impact relative to now
    const nowISO = new Date().toISOString();
    const withBlackout = items.map((i) => {
      if (i.impact === "High") {
        const w = blackout(i.time, 90);
        const inWindow = nowISO >= w.from && nowISO <= w.to;
        return { ...i, isBlackout: inWindow };
      }
      return i;
    });

    // compute bias (per currency; filter to requested currencies if supplied)
    ...
    if (currencies.length === 2) {
      ...
      const raw = Math.round((b - q) * 10) / 10;
      const score = clamp(raw, -5, 5);
      instrumentBias = {
        pair: `${base}${quote}`,
        score,
        label: biasLabel(Math.round(score)),
      };
    }
    ...
    return res
      .status(200)
      .json({ ok: false, reason: e?.message || "calendar error" });
  }
}
